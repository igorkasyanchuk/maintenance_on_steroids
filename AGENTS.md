...
- Any third-party CDN asset in a privileged engine view carries a `sha384` SRI `integrity=` plus `crossorigin="anonymous"`, with the version single-sourced in an ERB local so the script src, stylesheet URL, and hash can't drift — see `docs/solutions/best-practices/subresource-integrity-for-cdn-assets.md`.
